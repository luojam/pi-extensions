            ...
            return {
                ...
                render(width: number): string[] {
                    ...
                    const frameLine = (content: string, top: boolean): string => {
                        ...
                        return (
                            theme.fg('dim', leftBorder) +
                            content +
                            theme.fg('dim', fill + rightBorder)
                        );
                    };
                    ...
                    const contextText =
                        contextTokens === null || contextTokens === undefined
                            ...
                            : `(${formatPercentWithoutDecimal(contextPercent)}%) ${formatTokens(contextTokens)}/${formatTokens(contextWindow)}`;
                    ...
                    if (
                        visibleWidth(cwd) + 2 + visibleWidth(modelDisplay) > contentWidth &&
                        ctx.model
                    ) {
                    ...
                    const cwdLine = alignContent(theme.fg('dim', cwd), modelDisplay);
                    const statsLine = alignContent(left, '');
                    const lines = [frameLine(cwdLine, true), frameLine(statsLine, false)];
...
function formatPercentWithoutDecimal(percent: number | null | undefined): string {
